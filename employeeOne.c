...
PtrToEmployee searchEmployeeByNumber(PtrToConstEmployee ptr, int tableSize, long targetNumber) { 
    const PtrToConstEmployee endPtr = ptr + tableSize; //find end of the array--don't go past end

    for(; ptr < endPtr; ptr++)  //search until end of table  ptr++ will increment by what?? by 32 Bytes
    { 
        //look at row and defrence the number inside struct pointer.---- the notation is -> for dereferncing
